...
вимоги до АЗ ПЗ
### Інструкція
#### 1. Розгортання elasticsearch - слідуйте [інструкціям](https://www.elastic.co/elasticsearch)
- створення індексів для ваших мереж, напкриклад:
```json
PUT telegram
{
  "mappings": {
...
```
- створення ідкексу для рейтингу каналів
```json
PUT channel_rating
```
#### 2. Запуск парсерів
- у файлі ```base/modules.py``` замініть шлях на основний шлях вашого проекту
- установіть залежності
```
pip install -r requirenments.txt
```
- шалаштуйте файл у ```.env``` у диреккторії ```base```
```
ELASTICSEARCH_USER=elastic
ELASTICSEARCH_PASSWORD=<Ваш пароль>
SLACK_BOT_TOKEN=<токер слак бота - для репортів щодо сканування>
CHANNEL_ID_REPORT=<id каналу в slack>
OPENAI_API_KEY=<токен gpt api>
```
- Запустіть файли парсингу у cron linux, наприклад:
```
crontab -e
```
```
* 2 * * * /usr/bin/python3 /path/to/your_script.py >> /path/to/logfile.log 2>&1
```
```
crontab -l
```
файли парсингу знаходяться у відповідних директоріях з назвами самих мереж (telegram)

#### 3. Запуск сайту
- у папці ```site_project``` ввести наступну команду
```
docker-compose up --build
```

### Приємного користування системою))
